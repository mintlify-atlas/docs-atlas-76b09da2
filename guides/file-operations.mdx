...
`write_json` only needs to communicate success or failure — there is no result value to return. Using `BasicReturn` (aliased as `BaseReturn` in the import) keeps the return type minimal and makes the function's intent immediately clear to anyone reading the code.
